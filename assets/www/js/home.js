...
function onStartJourneySuccess(position) { //Position object created by the geolocation API: coords + timestamp
    ...
    addJourneyToDB(journeyName, startTime); //Write journey name to db when start journey button is clicked.
    addTrackPointToDB(journey_id, position) //Write geolocation to db.

}

function onStartError(error) {
    alert('Start Journey Error');
}
...
function onEndJourneySuccess(position) { //Position object created by the geolocation API: coords + timestamp
    //Write to db using model.js functions.
    endJourneyInDB(journeyName, endTime); //Write journey name to db when stop journey button is clicked.
    addTrackPointToDB(journey_id, position) //Write geolocation to db.
}
...
$("#takePhoto").on('click', function() {
    ...
    photoWatchId = navigator.geolocation.watchPosition(onStartJourneySuccess, onStartError, { // Start tracking
    frequency: 30000,
    // maximumAge: 3000,
    // timeout: 5000,
    enableHighAccuracy: true
