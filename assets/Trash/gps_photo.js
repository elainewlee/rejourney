...
document.addEventListener("deviceready", function(){
    ...
    if(!db){
        db = window.openDatabase("../../../mnt/sdcard/rejourneyDatabase", "1.0", "ReJourney App Database", 200000); //This method will create a new SQL Lite Database and return a Database object.
    }
...
function readSQL(fileToRead, transaction) {
    window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, function (fileSystem) { //Fires if we have permissions to access the filesystem, creates object
        fileSystem.root.getFile(fileToRead, null, function (fileEntry) { //getFile is a method that acts on filesystem object to get fileToRead.
            fileEntry.file(function (fileToRead) {  //Method reads file if file exists and passes
                ...
                reader.onloadend = function(evt) { //Function that gets called once the reader is done reading
                    ...
                    db.executeSql(createSQLSchema); // transaction.executeSql(createSQLSchema); // The transaction object accepts the SQL string argument and executes it against the database
